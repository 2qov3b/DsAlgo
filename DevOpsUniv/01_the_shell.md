...
[devops.kenshen@gmail.com](mailto:devops.kenshen@gmail.com).
...
interface: [The Shell](https://en.wikipedia.org/wiki/Shell_(computing)).
...
In this lecture, we will focus on the Bourne Again SHell, or ["bash"](https://en.wikipedia.org/wiki/Bash_(Unix_shell)) for
...
are on the machine `ip-xx-xx-xx-xx` and that your "current working directory",
...
```console
[ec2-user@ip-xx-xx-xx-xx ~]$ echo hello
hello
```
...
```console
$ ls -l /home
drwxr-xr-x 1 ec2-user  users  4096 Jun 15  2019 ec2-user
```
...
`ec2-user` is a directory. Then follow three groups of three characters
(`rwx`). These indicate what permissions the owner of the file
(`ec2-user`), the owning group (`users`), and everyone else respectively
...
modify (`w`) the `ec2-user` directory (i.e., add/remove files in it). To
...
The simplest form of [redirection](https://en.wikipedia.org/wiki/Redirection_(computing)) is `< file` and `> file`. These let you
...
input/output redirection really shines is in the use of [pipes](https://en.wikipedia.org/wiki/Pipeline_(Unix)). The `|`
...
A [named pipe](https://en.wikipedia.org/wiki/Named_pipe), however, can last as long as the system is up, beyond the life of the process. It can be deleted if no longer used.
...
file system mounted under `/sys`. `sysfs` exposes a number of [kernel](https://en.wikipedia.org/wiki/Kernel_(operating_system))
...
# Exercises
 ...
 1. Create a new directory called `devops` under `/tmp`.
 2. Look up the `touch` program. The `man` program is your friend.
 3. Use `touch` to create a new file called `test` in `devops`.
 ...
 7. Use `chmod` to make it possible to run the command `./test`.
 8. Use `|` and `>` to write the "last modified" date output by
    `test` into a file called `last-modified.txt` in your home
    directory.
